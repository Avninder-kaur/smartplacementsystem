...
import { LayoutDashboard, PlusCircle, FolderOpen, Users, Building2, FileText, Search, Calendar, } from "lucide-react";
...
const allStatuses = ["All", "Pending", "Shortlisted", "Interview Scheduled", "Offered", "Rejected"];
export default function ViewApplicants() {
    ...
    const counts = allStatuses.slice(1).reduce((acc, s) => {
    ...
    return (<DashboardLayout navItems={navItems} title="Recruiter Portal">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <div className="relative max-w-xs w-full">
          ...
          <Input placeholder="Search name, university, skill..." className="pl-9" value={search} onChange={(e) => setSearch(e.target.value)}/>
        </div>
      </div>

      {/* Summary chips */}
      <div className="flex flex-wrap gap-2 mb-4">
        {allStatuses.map((s) => (<button key={s} onClick={() => setStatusFilter(s)} className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${statusFilter === s
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {allJobs.map((j) => (<button key={j} onClick={() => setJobFilter(j)} className={`px-3 py-1 rounded-md text-xs font-medium border transition-colors ${jobFilter === j
      ...
      {filtered.length === 0 ? (<div className="bg-card border rounded-xl p-12 text-center">
          ...
          {filtered.map((a, i) => (<motion.div key={a.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }} className="bg-card border rounded-xl p-5 hover:shadow-lg hover:shadow-primary/5 transition-all duration-300">
              ...
              {a.interviewDate && (<p className="text-xs text-cyan-400 mb-2 flex items-center gap-1">
                  <Calendar className="h-3 w-3"/> Interview: {a.interviewDate}
                </p>)}

              <div className="flex flex-wrap gap-1.5 mb-4">
                {a.skills.map((s) => (<Badge key={s} variant="outline" className="text-xs font-normal">{s}</Badge>))}
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {a.status === "Pending" && (<>
                    ...
                    <Button size="sm" variant="ghost" className="text-destructive hover:text-destructive" onClick={() => updateStatus(a.id, "Rejected")}>
                      Reject
                    </Button>
                  </>)}
                ...
                {(a.status === "Rejected" || a.status === "Offered") && (<Button size="sm" variant="ghost" onClick={() => updateStatus(a.id, "Pending")}>
                    Reset to Pending
                  </Button>)}
              </div>
            </motion.div>))}
              ...
              <div className="space-y-4 mt-2">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {selectedApp.skills.map((s) => (<Badge key={s} variant="outline" className="text-xs">{s}</Badge>))}
                  </div>
                </div>
      ...
      <Dialog open={!!scheduleApp} onOpenChange={() => setScheduleApp(null)}>
        <DialogContent className="sm:max-w-sm">
          ...
          {scheduleApp && (<form onSubmit={handleScheduleInterview} className="space-y-4 mt-2">
