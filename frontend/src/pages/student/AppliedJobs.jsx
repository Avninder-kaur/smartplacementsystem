...
import { LayoutDashboard, User, Briefcase, FileCheck, Activity, Bell, MapPin, IndianRupee, } from "lucide-react";
...
const statusFilters = ["Pending", "Shortlisted", "Rejected", "Interview Scheduled"];
    ...
    return (<DashboardLayout navItems={navItems} title="Student Portal">
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        ...
        {statusFilters.map((s) => {
            ...
            return (<Button key={s} size="sm" variant={filter === s ? "default" : "outline"} onClick={() => setFilter(s)}>
              {s} ({count})
            </Button>);
        })}
      </div>
      ...
      {filtered.length === 0 ? (<div className="bg-card border rounded-xl p-12 text-center">
          ...
          {filtered.map((app, i) => (<motion.div key={app.job.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="bg-card border rounded-xl p-5 flex flex-col cursor-pointer hover:shadow-lg hover:shadow-primary/5 transition-all duration-300" onClick={() => setSelectedApp(app)}>
              ...
              <div className="flex flex-wrap gap-1.5 mb-3">
                {app.job.skills.map((s) => (<Badge key={s} variant="outline" className="text-xs font-normal">{s}</Badge>))}
              </div>
              ...
              <div className="space-y-4 mt-2">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {selectedApp.job.skills.map((s) => (<Badge key={s} variant="outline" className="text-xs">{s}</Badge>))}
                  </div>
                </div>
              </div>
            </>)}
