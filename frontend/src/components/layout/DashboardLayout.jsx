...
export function DashboardLayout({ children, navItems, title }) {
    ...
    const dashboardItem = navItems.find((item) => item.label.toLowerCase().includes("dashboard"));
    const profileItem = navItems.find((item) => item.label.toLowerCase().includes("profile"));
    const dashboardUrl = dashboardItem?.to ?? "/";
    const profileUrl = profileItem?.to ?? dashboardUrl;
    const profilePic = user?.profilePic && user.profilePic.startsWith("http")
        ? user.profilePic
        : `${API_BASE_URL.replace("/api", "")}${user?.profilePic || ""}`;
    return (<div className="min-h-screen flex bg-background">
      {/* Sidebar overlay for mobile */}
      <AnimatePresence>
        {sidebarOpen && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-background/60 backdrop-blur-sm z-40 lg:hidden" onClick={() => setSidebarOpen(false)}/>)}
      </AnimatePresence>

      {/* Sidebar */}
      <aside className={`fixed lg:sticky top-0 left-0 z-50 h-screen w-64 bg-card border-r flex flex-col transition-transform duration-300 lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <nav className="flex-1 py-4 px-3 space-y-1 overflow-y-auto">
          {navItems.map((item) => {
            ...
            return (<Link key={item.to} to={item.to} onClick={() => setSidebarOpen(false)} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 ${isActive
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="sticky top-0 z-30 h-16 flex items-center justify-between px-4 md:px-6 border-b bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/80">
          <button className="lg:hidden p-2 -ml-2 rounded-md hover:bg-accent" onClick={() => setSidebarOpen(true)}>
          ...
          <div className="flex items-center gap-1">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                {user?.profilePic ? (<img src={profilePic} alt={user.name} className="ml-2 h-9 w-9 rounded-full object-cover ring-2 ring-primary/20 hover:ring-primary/40 transition-all cursor-pointer"/>) : (<button className="ml-2 h-9 w-9 rounded-full bg-primary/15 flex items-center justify-center text-sm font-semibold text-primary ring-2 ring-primary/20 hover:ring-primary/40 transition-all cursor-pointer">
