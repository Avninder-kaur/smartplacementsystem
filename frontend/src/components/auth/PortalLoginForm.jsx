...
const defaultRedirect = (role) => {
  // Route recruiters to the company/recruiter dashboard, admins to admin dashboard
  if (role === "admin") return "/admin/dashboard";
  if (role === "recruiter" || role === "company") return "/recruiter/dashboard";
...
export default function PortalLoginForm({
  ...
  const redirectByRole = (nextRole) => {
    navigate(defaultRedirect(nextRole), { replace: true });
  };
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      redirectByRole(response.user?.role || role);
    }
    catch (error) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-sm text-muted-foreground font-body">{subtitle}</p>
          {notice ? (
        ...
        <form onSubmit={handleLogin} className="bg-card border rounded-xl p-6 space-y-5">
          {showRoleSelect ? (
            <div className="space-y-2">
              ...
              <select id="role" value={role} onChange={(e) => setRole(e.target.value)} className="w-full h-10 border bg-background rounded-md px-3 text-sm">
          ...
          <Button type="submit" className="w-full" disabled={submitting}>
            {submitting ? "Signing In..." : loginButtonLabel}
          </Button>
