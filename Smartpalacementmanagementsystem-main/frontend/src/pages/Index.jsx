    ...
    return (<div className="min-h-screen bg-background">
      ...
      <section className="container py-20 md:py-32">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.15 }} className="mt-6 text-lg md:text-xl text-muted-foreground font-body max-w-2xl mx-auto">
            A single source of truth for students, recruiters, and placement
            officers. Apply, recruit, and manage — all in one professional
            platform.
          </motion.p>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }} className="mt-10 flex flex-col sm:flex-row gap-4 justify-center">
      ...
      <section id="features" className="border-t bg-card py-20">
        <div className="container">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {features.map((f, i) => (<motion.div key={f.title} custom={i} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={fadeUp} className="bg-background border rounded-lg p-6 hover:shadow-md transition-shadow">
      ...
      <section id="stats" className="py-20 container">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
          {stats.map((s, i) => (<motion.div key={s.label} custom={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="text-center">
