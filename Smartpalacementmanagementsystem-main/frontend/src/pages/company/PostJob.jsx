...
import { LayoutDashboard, PlusCircle, FolderOpen, Users, Building2, } from "lucide-react";
...
export default function PostJob() {
    ...
    return (<DashboardLayout navItems={navItems} title="Recruiter Portal">
      ...
      <form ref={formRef} className="bg-card border rounded-xl p-6 max-w-2xl space-y-5" onSubmit={handleSubmit}>
        {/* Job Type toggle */}
        <div className="space-y-2">
          <Label>Job Type <span className="text-destructive">*</span></Label>
          <div className="flex flex-wrap gap-2">
            {jobTypes.map((type) => (<button key={type} type="button" onClick={() => setJobType(type)} className={`px-4 py-2 rounded-lg text-sm font-medium border transition-all ${jobType === type
        ...
        <div className="space-y-2">
          ...
          <Textarea id="description" name="description" placeholder="Describe the role, responsibilities, and what you're looking for..." rows={4} required/>
        </div>

        <div className="space-y-2">
          <Label>Required Skills <span className="text-destructive">*</span></Label>
          <SkillsTagInput value={skills} onChange={setSkills} placeholder="Type a skill and press Enter..."/>
          {skills.length === 0 && (<p className="text-xs text-muted-foreground">Add skills like React, Python, SQL…</p>)}
        </div>
