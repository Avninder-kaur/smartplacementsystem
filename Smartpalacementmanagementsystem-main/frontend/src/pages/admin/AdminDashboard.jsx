...
import { LayoutDashboard, Users, Building2, Briefcase, FileText, TrendingUp, Award, } from "lucide-react";
...
export default function AdminDashboard() {
    const { user } = useAuth();
    return (<DashboardLayout navItems={navItems} title="Admin Panel (TPO)">
      <h1 className="text-2xl font-bold mb-2">Welcome back, {user?.name || "Admin"}!</h1>
      <p className="text-muted-foreground font-body mb-8">Admin dashboard showing placement overview and campus statistics</p>
      ...
      <div className="grid lg:grid-cols-2 gap-6 mb-6">
        <div className="bg-card border rounded-xl p-6">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={barData}>
              ...
              <Tooltip contentStyle={{ backgroundColor: "hsl(217, 33%, 17%)", border: "1px solid hsl(215, 25%, 22%)", borderRadius: "8px", color: "hsl(210, 40%, 96%)" }}/>
        ...
        <div className="bg-card border rounded-xl p-6">
          ...
          <div className="flex justify-center gap-6 mt-2">
            {pieData.map((entry, i) => (<div key={entry.name} className="flex items-center gap-2 text-sm">
